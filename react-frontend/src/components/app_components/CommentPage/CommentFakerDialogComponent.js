...
import commentFakerFactory from "./commentFakerFactory";
...
const CommentFakerDialogComponent = (props) => {
...
const [taskIds, settaskIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        .find({ query: { $select: ["_id","displayName"] } })
                        .then((data) => {
                        setauthorIds(data.data);
                        });
client.service("comment")
                        .find({ query: { $select: ["_id","content"] } })
                        .then((data) => {
                        setparentCommentIds(data.data);
                        });
client.service("task")
    ...
    const onRun = async () => {
        let fakeData = commentFakerFactory(props.user,fakerCount,authorIds,parentCommentIds,taskIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("comment").create(elem));
...
export default connect(mapState, mapDispatch)(CommentFakerDialogComponent);
