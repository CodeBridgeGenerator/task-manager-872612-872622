...
import projectFakerFactory from "./projectFakerFactory";
...
const ProjectFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("project").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectFakerDialogComponent);
