...
const AuditsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            ...
            for (let i = 0; i < seederData.length; i++) {
                const res = await SeederService('audits', seederData[i], promises, services, _schema, props.user);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
    ...
    return (
        <Dialog header="Seeder" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: '40vw' }} className="min-w-max zoomin animation-duration-700" footer={renderFooter()} resizable={false}>
            <div>
