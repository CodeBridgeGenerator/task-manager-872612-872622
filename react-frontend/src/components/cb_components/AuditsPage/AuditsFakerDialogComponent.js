...
const AuditsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        setLoading(true);
        const promises = fakeData.map((elem) => client.service('audits').create(elem));
    ...
    return (
        <Dialog header="Faker" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: '40vw' }} className="min-w-max zoomin animation-duration-700" footer={renderFooter()} resizable={false}>
            <div>
                <div>
