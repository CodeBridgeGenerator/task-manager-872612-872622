...
import FcmMessageDatatable from './FcmMessageDataTable';
...
import FcmMessageFakerDialogComponent from './FcmMessageFakerDialogComponent';
import FcmMessageSeederDialogComponent from './FcmMessageSeederDialogComponent';
import SortIcon from '../../../assets/media/Sort.png';
import FilterIcon from '../../../assets/media/Filter.png';

const FcmMessagePage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema('fcmMessages');
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({
                query: {
                    $limit: 10000,
                    recipients: urlParams.singleUsersId,
            ...
            .catch((error) => {
                ...
                props.alert({
                    title: 'Fcm Message',
                    type: 'error',
                    message: error.message || 'Failed get Fcm Message'
                });
            });
    ...
    const deleteRow = async () => {
        try {
            await client.service('fcmMessages').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('fcmMessages').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
    ...
    return (
        <div className="mt-5">
            <div className="grid">
                ...
                <div className="col-6 flex justify-content-end">
                    <>
                        ...
                        ></SplitButton>
                        <Button label="add" style={{ height: '30px' }} rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="fcmMessage-add-button" />
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="fcmMessage-datatable">
                    <FcmMessageDatatable
                        items={data}
                        fields={fields}
            ...
            <FcmMessageEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <FcmMessageCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} />
            <FcmMessageFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <FcmMessageSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(FcmMessagePage);
