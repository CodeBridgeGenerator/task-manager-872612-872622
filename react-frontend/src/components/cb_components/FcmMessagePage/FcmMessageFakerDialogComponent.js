...
import fcmMessageFakerFactory from './fcmMessageFakerFactory';
...
const FcmMessageFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fcmMessageFakerFactory(props.user, fakerCount, recipientsIds, fromIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('fcmMessage').create(elem));
...
export default connect(mapState, mapDispatch)(FcmMessageFakerDialogComponent);
