...
const PositionsPage = (props) => {
    ...
    const onRowClick = ({ data }) => {
        navigate(`/cbAdmin/positions/${data._id}`);
    };
    ...
    const get = async () => {
        ...
        if (currentCache && selectedUser) {
            ...
            if (selectedUserProfile) {
                const paginatorRecordsNo = _.get(selectedUserProfile, 'preferences.settings.positions.paginatorRecordsNo', 10);
        ...
        try {
            ...
            const paginatorRecordsNo = _.get(profileResponse, 'preferences.settings.positions.paginatorRecordsNo', 10);
            setPaginatorRecordsNo(paginatorRecordsNo);
        ...
        const updateCache = async () => {
            ...
            if (currentCache && selectedUser) {
                ...
                if (selectedUserProfileIndex !== -1) {
                    _.set(currentCache.profiles[selectedUserProfileIndex], 'preferences.settings.positions.paginatorRecordsNo', paginatorRecordsNo);
    ...
    return (
        <div className="mt-5">
            ...
            <PositionsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} />
