...
const CompaniesPage = (props) => {
    ...
    const onRowClick = ({ data }) => {
        navigate(`/cbAdmin/companies/${data._id}`);
    };

    const handleEditNavigation = (rowData) => {
        navigate(`/cbAdmin/companies/edit/${rowData._id}`);
    };

    const handleAddNavigation = () => {
        navigate('/cbAdmin/companies/create');
    };
    ...
    const get = async () => {
        ...
        if (currentCache && selectedUser) {
            ...
            if (selectedUserProfile) {
                const paginatorRecordsNo = _.get(selectedUserProfile, 'preferences.settings.companies.paginatorRecordsNo', 10);
        ...
        try {
            ...
            const paginatorRecordsNo = _.get(profileResponse, 'preferences.settings.companies.paginatorRecordsNo', 10);
            setPaginatorRecordsNo(paginatorRecordsNo);
        ...
        const updateCache = async () => {
            ...
            if (currentCache && selectedUser) {
                ...
                if (selectedUserProfileIndex !== -1) {
                    _.set(currentCache.profiles[selectedUserProfileIndex], 'preferences.settings.companies.paginatorRecordsNo', paginatorRecordsNo);
        ...
        <div className="mt-5">
            <div className="grid">
                ...
                <div className="col-6 flex justify-content-end">
                    <>
                        <FavouriteService favouriteItem={favouriteItem} serviceName="companies" />{' '}
                        <SplitButton
                        ...
                        ></SplitButton>
                        {permissions.create ? <Button label="add" style={{ height: '30px', marginRight: '10px' }} rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="companies-add-button" /> : null}
                    </>
                </div>
            </div>
