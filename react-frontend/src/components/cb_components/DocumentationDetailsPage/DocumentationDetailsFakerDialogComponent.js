...
import documentationDetailsFakerFactory from './documentationDetailsFakerFactory';
...
const DocumentationDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [documentationFileIds, setdocumentationFileIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                setdocumentationFileIds(data.data);
            });
    ...
    const onRun = async () => {
        let fakeData = documentationDetailsFakerFactory(props.user, fakerCount, documentationFileIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('documentationDetails').create(elem));
...
export default connect(mapState, mapDispatch)(DocumentationDetailsFakerDialogComponent);
