...
import seederData from './documentationDetails.seeder.json';
...
const DocumentationDetailsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            ...
            for (let i = 0; i < seederData.length; i++) {
                const res = await SeederService('documentationDetails', seederData[i], promises, services, _schema, props.user);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(DocumentationDetailsSeederDialogComponent);
