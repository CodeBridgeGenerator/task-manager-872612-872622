...
import NotificationTemplatesDatatable from './NotificationTemplatesDataTable';
import NotificationTemplatesEditDialogComponent from './NotificationTemplatesEditDialogComponent';
import NotificationTemplatesCreateDialogComponent from './NotificationTemplatesCreateDialogComponent';
import NotificationTemplatesFakerDialogComponent from './NotificationTemplatesFakerDialogComponent';
import NotificationTemplatesSeederDialogComponent from './NotificationTemplatesSeederDialogComponent';
import SortIcon from '../../../assets/media/Sort.png';
import FilterIcon from '../../../assets/media/Filter.png';

const NotificationTemplatesPage = (props) => {
    ...
    const filename = 'notificationTemplates';
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema('notificationTemplates');
        ...
        client
            ...
            .catch((error) => {
                ...
                props.alert({
                    ...
                    type: 'error',
                    message: error.message || 'Failed get Notification Templates'
                });
            });
    ...
    const deleteRow = async () => {
        try {
            await client.service('notificationTemplates').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('notificationTemplates').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
    ...
    const onRowClick = ({ data }) => {
        navigate(`/notificationTemplates/${data._id}`);
    };
    ...
    return (
        <div className="mt-5">
            <div className="grid">
                ...
                <div className="col-6 flex justify-content-end">
                    <>
                        ...
                        ></SplitButton>
                        <Button label="add" style={{ height: '30px' }} rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="notificationTemplates-add-button" />
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="notificationTemplates-datatable">
            ...
            <NotificationTemplatesEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <NotificationTemplatesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} />
            <NotificationTemplatesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <NotificationTemplatesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(NotificationTemplatesPage);
