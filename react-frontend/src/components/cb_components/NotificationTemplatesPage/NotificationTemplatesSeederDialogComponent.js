...
import seederData from './notificationTemplates.seeder.json';
...
const NotificationTemplatesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            ...
            for (let i = 0; i < seederData.length; i++) {
                const res = await SeederService('notificationTemplates', seederData[i], promises, services, _schema, props.user);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(NotificationTemplatesSeederDialogComponent);
