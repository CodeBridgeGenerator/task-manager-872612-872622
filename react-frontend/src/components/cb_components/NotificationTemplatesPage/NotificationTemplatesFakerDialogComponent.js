...
import notificationTemplatesFakerFactory from './notificationTemplatesFakerFactory';
...
const NotificationTemplatesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = notificationTemplatesFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('notificationTemplates').create(elem));
...
export default connect(mapState, mapDispatch)(NotificationTemplatesFakerDialogComponent);
